...
def process_claim(fabric, claim_regex, raw_claim):
    ...
    logging.info('Claim {} @ {},{}: {}x{}'.format(claim_id, left, top, width, height))
...
def main(argv):
    ...
    overlaps = [v for v in fabric.values() if len(v) > 1]
    print('There are {} overlaps in claims'.format(len(overlaps)))
