...
def main(argv):
    ...
    for guard, hour in sleep_time.items():
        logging.debug('{}: {}'.format(guard, hour))
        mins_asleep = sum(hour)
        if mins_asleep > sleepiest_mins:
            sleepiest_guard = guard
            sleepiest_mins = mins_asleep
    logging.info('Guard {} spent {} minutes asleep'.format(sleepiest_guard, sleepiest_mins))
    sleepiest_minute = -1, -1
    for m in range(len(sleep_time[sleepiest_guard])):
        if sleep_time[sleepiest_guard][m] > sleepiest_minute[1]:
            sleepiest_minute = m, sleep_time[sleepiest_guard][m]
    logging.info('Guard {} sleepiest minute is {} with {} occurrences'.format(sleepiest_guard, sleepiest_minute[0], sleepiest_minute[1]))
    answer = sleepiest_guard * sleepiest_minute[0]
    logging.info('Answer: Guard {} * minute {} = {}'.format(sleepiest_guard, sleepiest_minute[0], answer))
